 ...
 * @param x0 coordinates of window
 * @param y0 coordinates of window
 * @param x1 coordinates of window
 * @param y1 coordinates of window
 * @return none
 */
static void ST7789_SetAddressWindow(const uint16_t x0, const uint16_t y0, const uint16_t x1, const uint16_t y1)
{
    ST7789_Select();
    const uint16_t x_start = x0 + X_SHIFT;
    const uint16_t x_end = x1 + X_SHIFT;
    const uint16_t y_start = y0 + Y_SHIFT;
    const uint16_t y_end = y1 + Y_SHIFT;
...
#ifdef INVON
    ST7789_WriteCommand(ST7789_INVON); //	Inversion OFF
#else
...
void ST7789_Fill_Color(uint16_t color)
{
    ...
    ST7789_Select();
    const uint16_t convert_color = ((color & 0xFF) << 8) | ((color & 0xFF00) >> 8);
#ifdef USE_DMA
 ...
 * @param xSta coordinate of the start point
 * @param ySta coordinate of the start point
 * @param xEnd coordinate of the end point
 * @param yEnd coordinate of the end point
 * @param color -> color to Fill with
 * @return none
 */
